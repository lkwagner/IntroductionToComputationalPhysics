...
# 
# Start by initializing a few things, then executing a loop that calculates the nth term, with n running from 0 to 999,999, summing the terms as you go. Here’s a flowchart for one way to structure your program...
# 

# <img src="attachment:tan_pi.png" alt="Flowchart for finding pi" style="width: 500px;"/>

# The other option is to write this in a line or two using list comprehensions. 
# 
# You may use either of these two options. 
...
# 
# 
# <div><img src="https://clark.physics.illinois.edu/246img/AnsStart5.svg" width=200 align=left></img><br></div>
...
# <div><img src="https://clark.physics.illinois.edu/246img/AnsEnd.svg" width=200 align=left></img><br></div>

# ### b. List Introspection
...
# You'll probably find that the terms in the series look pretty boring. That's because they get small really fast! To see them a little better we can adjust the y-axis to be scalled *logarithmically* rather than *linearly*, by calling `plt.yscale('log')`. (This will be explored later in the course!)

# *Write your code (and generate your plot) below.*

# <div><img src="https://clark.physics.illinois.edu/246img/AnsStart.png" width="400" align=left alt="Answer (start)"></img></div>

# In[6]:


seriesTerms = [8.0/( (4*n+3)*(4*n+1)) for n in range(0,100000)] #!#
plt.plot(range(0,100000),seriesTerms) #!#
plt.xlabel("Term")
plt.ylabel("Value")
plt.yscale('log')
plt.show()
...
# <div>
#     <img src="https://clark.physics.illinois.edu/246img/end.png" width=640 align=left></img><br>
# </div>
# 
# 
# &#x2611;
...
# <div>
#     <img src="https://clark.physics.illinois.edu/246img/start.png" width=640 align=left></img><br><br>
# </div>
# 
# 
...
for j in range(1,4):#!#
    print(np.log(np.abs(1.0/(2*np.sqrt(2)/9801*np.sum(np.array([np.math.factorial(4*k)*(1103+26390*k)/(np.math.factorial(k)**4*396**(4*k)) for k in range(0,j)])))-np.pi))) #!#
...
def DrawCircle():
    r = 1
    #add code to generate x,y here
    #then plot your lists/arrays with plt.plot
    dx=0.0001#!#
    x=np.arange(-1,1,dx)#!#
    x_r=np.arange(1,-1,-dx)#!#
    y=list(np.sqrt(np.abs(1-x**2)))+list(-1*np.sqrt(1-x_r**2))+[np.sqrt(np.abs(1-x[0]**2))]#!#
    x=list(x)+list(x_r)+[x[0]]#!#
    plt.plot(x,y,'--')#!#
    perimeter=0#!#
    for i in range(0,len(x)-1):#!#
        h=( (x[i+1]-x[i])**2 + (y[i+1]-y[i])**2 )**0.5#!#
        perimeter+=h#!#
...
# 
# <div><img src="https://clark.physics.illinois.edu/246img/start.png" width=640 align=left></img></div>
...
def DrawPolygon(N):
    ...
    pointsY=[]#!#
    perimeter=0.0#!#
    for theta in np.arange(0,2*np.pi+1e-5,dTheta):#!#
        pointsX.append(np.cos(theta))#!#
        pointsY.append(np.sin(theta))#!#
    plt.plot(pointsX,pointsY,'-o')#!#
    for i in range(0,len(pointsX)-1):#!#
        perimeter+=((pointsX[i]-pointsX[i+1])**2+(pointsY[i]-pointsY[i+1])**2)**0.5#!#
    return perimeter#!#
    return 0. # fixed in c.

DrawCircle()
print(DrawPolygon(5)/2.)
plt.show()
...
# 
# 
# Part b using list comprehensions!
# 
# 
...
resetMe(['DrawCircle','DrawPolygon'])
DrawCircle()
poly=[]#!#
...
# <div><img src="https://clark.physics.illinois.edu/246img/extracredit.png" width=640 align=left></img></div>
# 
# 
# 
...
def DrawOuterPolygon(N): 
    dtheta=2*np.pi/N#!#
    r=np.sqrt(1**2+np.tan(dtheta/2)**2)#!#
    theta=np.arange(0,2*np.pi+1e-5,dtheta)+dtheta/2#!#
    x=r*np.cos(theta)#!#
    y=r*np.sin(theta)#!#
    plt.plot(x,y,'o-')#!#
    perimeter=0#!#
...
Ns=range(10,128)#!#
for N in Ns:#!#
    data1.append(DrawPolygon(N)/2.)#!#
    data2.append(DrawOuterPolygon(N)/2.)#!#
plt.show()#!#
...
def GetPi(steps=2500):#!#
    ...
    while tries<steps:#!#
        ...
        if x**2+y**2<1:#!#
            inside+=1#!#
        tries+=1#!#
    return inside/tries*4#!#
storeHist=[]#!#
for i in range(0,1000):#!#
    storeHist.append(GetPi())#!#

storeHist2=[]#!#
for i in range(0,1000):#!#
    storeHist2.append(GetPi(10000))#!#
...
# <div>
#     <img src="https://clark.physics.illinois.edu/246img/extracredit.png" width=640 align=left></img><br>
# </div>
# 
# 
# Exercise 4 is extra credit. 
# 
# 
